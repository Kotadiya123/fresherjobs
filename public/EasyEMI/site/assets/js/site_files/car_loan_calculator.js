...
var carLoancalculatorApp = angular.module('calculatorApp', ['ngAnimate', 'ngSanitize','rzModule','chart.js','zingchart-angularjs']);
carLoancalculatorApp.filter('customcurrency', function() {
...
carLoancalculatorApp.filter('newcustomcurrency', function() {
  return function(value) {
  var val = Math.abs(value);
  if (val >= 10000000) {
  val = (val / 10000000) + ' Cr';
...
carLoancalculatorApp.controller('carLoancalculatorController',['$scope', '$http','$log', '$sce','$window','$timeout',function ($scope, $http,$log, $sce,$window,$timeout){
  $scope.homeLoanAmount=1000000;
  $scope.homeLoanAmount_num = 1000000;
  $scope.currentCalculatorId=1;
  $scope.CalculatorName="Car Loan Calculator";
  $scope.sipStep=1;
...
$scope.chartcolors= [ '#FDB45C', '#46BFBD'];
 ...
 $scope.futureValueOfHouse ="";
  ...
  $scope.homeLoanRate = 6.5;
  $scope.homeLoanRate1=$scope.homeLoanRate;
  $scope.r = "";
  $scope.homeLoanTenure = 5;
 ...
 /******change Step for******/
  ...
  $scope.changehomeLoanRateTenure=function(){
    ...
    $scope.showErrorYear =  false;
    $scope.showErrorRate = false; 
    if($scope.homeLoanTenure > 7){
      $scope.homeLoanTenure = 7;
    ...
    if($scope.homeLoanAmount === 0 || $scope.homeLoanAmount === '' || $scope.homeLoanAmount < 500000){
  ...
  $scope.changeValueForRange = function(){
    if($scope.homeLoanAmount_num !== '' && parseInt($scope.homeLoanAmount_num) >= 500000){
